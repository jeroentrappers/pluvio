...
- [x] **3.5 Deployment gate** — `research/tools/promote_checkpoint.py`
      (`gate`: two frozen-benchmark JSONs on the same sample set, adequate, not
      significantly worse on CSI@1/FSS3/RMSE at any lead, significantly better
      somewhere; `canary`: N paired nowcast npz, finite/plausible/wet-fraction
      within 3×/field RMSE ≤ 2 mm/h; `swap`: atomic replace keeping the
      previous file + a promotion record) and `tools/canary_tick.py` running as
      `pluvio-canary` every 5 min on hetz1 (live 2026-09-04): with
      `checkpoints/candidate.pt` present it pairs the candidate's nowcast with
      the served one per issue under /opt/pluvio/serve/canary/; no-op
      otherwise. Runbook: scp candidate → ≥30 min (6 pairs) → `gate` on the two
      benchmark JSONs → `canary --incumbent-dir …/incumbent --candidate-dir
      …/candidate` → `swap`. Note: the served checkpoint runs on the legacy
      100² store, so a v3 (192²) candidate is only gateable after the 1.9
      switch — the benchmark must be run on the store the candidate serves.
