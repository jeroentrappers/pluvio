...
# research-grid bounds (W,S,E,N) — attrs override when the day zarr carries them
...
def observed_on(valid_epoch: int, bounds, shape):
    ...
    tk = next((k for k in root.array_keys() if "time" in k.lower()), None)
    times = np.asarray(root[tk][:], dtype="int64")
    i = int(np.argmin(np.abs(times - valid_epoch)))
    if abs(int(times[i]) - valid_epoch) > 180:
        return None
    rate = np.asarray(root["rate"][i], dtype="float32")
    qb = list(root.attrs.get("bounds", QPE_BOUNDS))
    H, W = rate.shape
    w, s, e, n = bounds
    ...
    if not (0 <= c0 < c1 <= W and 0 <= r0 < r1 <= H):
        return None
    sub = np.nan_to_num(rate[r0:r1, c0:c1])
    return cv2.resize(sub, (shape[1], shape[0]), interpolation=cv2.INTER_AREA)
